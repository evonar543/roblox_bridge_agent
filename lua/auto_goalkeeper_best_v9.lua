-- RBA Auto Goalkeeper v9 - SMART AIR.
-- Standalone jump-aware, ping-aware goalkeeper controller.
-- Previous stable snapshot: v8.
--
...
local DEFAULT_AUTO_JUMP = config.autoJump ~= false
...
local JUMP_MIN_HEIGHT_GAP = math.clamp(
    tonumber(config.jumpMinHeightGap) or 0.65,
    0.25,
    2
)
local JUMP_HORIZONTAL_TOLERANCE = math.clamp(
    tonumber(config.jumpHorizontalTolerance) or 0.8,
    0.25,
    3
)
local JUMP_SAFETY_MARGIN = math.clamp(
    tonumber(config.jumpSafetyMargin) or 0.3,
    0,
    1
)
local JUMP_CONFIRM_TIMEOUT = math.clamp(
    tonumber(config.jumpConfirmTimeout) or 0.3,
    0.15,
    0.6
)
local JUMP_COOLDOWN = math.clamp(
    tonumber(config.jumpCooldown) or 0.8,
    0.5,
    2
)
...
local AgentUtilities = require(ReplicatedStorage.Shared.Libraries.AgentUtilities)
...
local state = {
    ...
    autoJump = DEFAULT_BEST_MODE or DEFAULT_AUTO_JUMP,
...
local function createGui()
    ...
    local bestButton = createInstance("TextButton", {
        ...
        Text = "BEST MODE: ON  //  PING + SPRINT + SMART JUMP",
...
renderGui = function(force)
    ...
    controls.subtitle.Text = state.bestMode
        ...
        or ((state.autoDive or state.autoJump) and "Adaptive save response // running"
    ...
    if not state.running then
        ...
        liveLabel = "STOPPED"
    elseif state.status == "THREAT" or state.status == "DIVING"
        or state.status == "JUMPING" then
    ...
    if prediction then
        ...
        controls.direction.Text = prediction.saveAction == "JUMP"
            and "JUMP"
            or safeText(prediction.direction)
    ...
    controls.bestButton.Text = state.bestMode
        and "BEST MODE: ON  //  PING + SPRINT + SMART JUMP"
...
function state.health()
    return {
        ...
        jumpAttempts = state.jumpAttempts,
        jumps = state.jumps,
        lastJumpAt = state.lastJumpAt,
        lastJumpToken = state.lastJumpToken,
        pendingJump = state.pendingJump and {
            token = state.pendingJump.token,
            requestedAt = state.pendingJump.requestedAt
        } or nil,
...
local function effectiveLeadSeconds(horizontalDistance, directionAlignment)
    ...
    local alignment = math.clamp(tonumber(directionAlignment) or 1, 0.35, 1)
    local movementTime = horizontalDistance
        and math.clamp(horizontalDistance / (leapVelocity * alignment), 0.08, leapLifetime)
...
local function calibrateJumpTiming(shot, crossingAt)
    if not shot.jumpStartedAt or not shot.jumpTravelSeconds then
        return
    end
    local timingError = crossingAt
        - (shot.jumpStartedAt + shot.jumpTravelSeconds)
    if math.abs(timingError) > 1.5 then
        return
    end
    local learning = state.learning
    learning.jumpStartupSeconds = math.clamp(
        learning.jumpStartupSeconds - timingError * 0.18,
        0,
        0.2
    )
    addEvent("jump_timing_calibrated", {
        token = shot.token,
        timingError = timingError,
        startupSeconds = learning.jumpStartupSeconds
    })
end
...
local function observeShot(ball, goal, kinematics, incoming)
    ...
    if nearGoalMouth then
        ...
        calibrateJumpTiming(shot, os.clock())
    end
...
local function projectedHalfExtent(part, axis)
    return math.abs(axis:Dot(part.CFrame.RightVector)) * part.Size.X / 2
        ...
        + math.abs(axis:Dot(part.CFrame.UpVector)) * part.Size.Y / 2
end
...
local function getStaminaAmount()
    local ok, amount = pcall(function()
        return SharedStates.Stamina.Amount:get()
    end)
    return ok and tonumber(amount) or nil
end

local function isAirborne(character, humanoid)
    local utilityResult
    pcall(function()
        utilityResult = AgentUtilities.IsAirborne(character)
    end)
    if type(utilityResult) == "boolean" then
...
local function getJumpProfile(humanoid)
    local gravity = math.max(Workspace.Gravity, 1)
    local jumpVelocity
    if humanoid.UseJumpPower then
        jumpVelocity = math.max(tonumber(humanoid.JumpPower) or 0, 0)
    else
        jumpVelocity = math.sqrt(math.max(
            0,
            2 * gravity * math.max(tonumber(humanoid.JumpHeight) or 0, 0)
        ))
    end
    ...
    return {
        ...
        apexSeconds = jumpVelocity / gravity,
        apexRise = jumpVelocity * jumpVelocity / (2 * gravity)
    }
end

local function canRequestJump(character, humanoid)
    if not state.autoJump or state.pendingJump or state.activeJump then
        return false, "jump inactive"
    end
    if LocalPlayer:GetAttribute("HasBall") == true then
        return false, "keeper has ball"
    end
    if not isHumanoidUsable(humanoid) or humanoid.SeatPart then
        return false, "humanoid unavailable"
    end
    if isAirborne(character, humanoid) then
        return false, "already airborne"
    end
    local stamina = getStaminaAmount()
    if stamina and stamina < 10 then
        return false, "stamina reserved"
    end
    return true
end

local function getJumpPlan(character, humanoid, ball, goal, prediction, effectiveSaveDistance, token)
    local unavailable = {
        recommended = false,
        reason = "not needed"
    }
    if not state.autoJump or not prediction or not goal then
        return unavailable
    end
    local hitbox = character:FindFirstChild("Hitbox", true)
    if not hitbox or not hitbox:IsA("BasePart") then
        unavailable.reason = "no keeper hitbox"
        return unavailable
    end
    local verticalAxis = Vector3.yAxis
    local standingTop = hitbox.Position.Y + projectedHalfExtent(hitbox, verticalAxis)
    local ballBottom = prediction.position.Y - projectedHalfExtent(ball, verticalAxis)
    local heightGap = ballBottom - standingTop
    if heightGap < JUMP_MIN_HEIGHT_GAP then
        unavailable.reason = "within standing reach"
        unavailable.heightGap = heightGap
        return unavailable
    end
    if effectiveSaveDistance > JUMP_HORIZONTAL_TOLERANCE then
        unavailable.reason = "lateral save"
        unavailable.heightGap = heightGap
        return unavailable
    end
    if state.jumpFailedToken == token then
        unavailable.reason = "jump failed this shot"
        unavailable.heightGap = heightGap
        return unavailable
    end
    local profile = getJumpProfile(humanoid)
    if not profile then
        unavailable.reason = "jump disabled"
        return unavailable
    end
    local requiredRise = heightGap + JUMP_SAFETY_MARGIN
    if requiredRise > profile.apexRise then
        unavailable.reason = "above jump apex"
        unavailable.heightGap = heightGap
        return unavailable
    end
    local discriminant = profile.velocity * profile.velocity
        - 2 * profile.gravity * requiredRise
    local travelSeconds = (profile.velocity - math.sqrt(math.max(0, discriminant)))
        / profile.gravity
    local networkLead = state.bestMode and state.network.compensationSeconds or 0
    local commitLead = math.clamp(
        travelSeconds + networkLead + state.learning.jumpStartupSeconds,
        0.14,
        profile.apexSeconds + networkLead
    )
    return {
        recommended = true,
        reason = "high central shot",
        heightGap = heightGap,
        requiredRise = requiredRise,
        apexRise = profile.apexRise,
        travelSeconds = travelSeconds,
        commitLead = commitLead
    }
end
...
local function updateWalkAssist(humanoid, root, ball, goal, prediction, incoming)
    ...
    local jumpActive = state.pendingJump or state.activeJump
        or isAirborne(root.Parent, humanoid)
    if not state.walkAssist or state.pendingDive or diveActive or jumpActive then
        state.phase = (diveActive or jumpActive) and "RECOVER" or state.phase
        stopWalking(
            diveActive and "dive active"
                or state.pendingDive and "dive pending"
                or jumpActive and "jump active"
...
local function checkPendingJump()
    local pending = state.pendingJump
    if pending then
        local humanoid = pending.humanoid
        local root = pending.root
        local elapsed = os.clock() - pending.requestedAt
        local humanoidState = humanoid and humanoid.Parent and humanoid:GetState()
        local confirmed = humanoidState == Enum.HumanoidStateType.Jumping
            or (humanoidState == Enum.HumanoidStateType.Freefall
                and root and root.AssemblyLinearVelocity.Y > 1)
        if confirmed then
            local confirmedAt = os.clock()
            state.pendingJump = nil
            state.activeJump = {
            ...
            state.lastJumpAt = confirmedAt
            state.learning.jumpStartupSeconds = state.learning.jumpStartupSeconds
                + (math.clamp(elapsed, 0, 0.2)
                    - state.learning.jumpStartupSeconds) * 0.25
            local shot = state.activeShot
            if shot and shot.token == pending.token then
                shot.jumpStartedAt = confirmedAt
                shot.jumpTravelSeconds = pending.travelSeconds
                shot.expectedCrossingAt = pending.expectedCrossingAt
            end
            state.phase = "COMMIT"
            state.status = "JUMP CONFIRMED"
            addEvent("jump_confirmed", {
                token = pending.token,
                confirmationMs = math.floor(elapsed * 1000),
                prediction = pending.prediction
            })
        ...
        if not humanoid or not humanoid.Parent or elapsed >= JUMP_CONFIRM_TIMEOUT then
            state.pendingJump = nil
            state.jumpFailedToken = pending.token
            state.lastError = not humanoid or not humanoid.Parent
                and "Character changed before jump could be verified"
                or "Normal jump did not begin within confirmation window"
            state.status = "JUMP FALLBACK ARMED"
            addEvent("jump_not_confirmed", {
                token = pending.token,
                error = state.lastError
            })
            renderGui(true)
    ...
    if not isAirborne(humanoid.Parent, humanoid)
        and os.clock() - active.startedAt >= 0.12 then
        state.activeJump = nil
        state.phase = "RECOVER"
        state.status = "JUMP RECOVERED"
        addEvent("jump_recovered", { token = active.token })
        renderGui(true)
    end
    return state.activeJump ~= nil
end

local function requestJump(character, humanoid, token, prediction, jumpPlan)
    local allowed, reason = canRequestJump(character, humanoid)
    if not allowed then
        state.status = "JUMP UNAVAILABLE: " .. safeText(reason)
        return false
    end
    local nowClock = os.clock()
    if nowClock - state.lastJumpAttemptAt < JUMP_COOLDOWN then
        state.status = "JUMP COOLDOWN"
        return false
    end
    state.lastJumpAttemptAt = nowClock
    state.lastJumpToken = token
    state.jumpAttempts = state.jumpAttempts + 1
    state.phase = "COMMIT"
    ...
    state.pendingJump = {
        ...
        travelSeconds = jumpPlan.travelSeconds,
        expectedCrossingAt = nowClock + prediction.time
    }
    stopWalking("jump requested")
    local ok, errorMessage = pcall(function()
        MovementController:UpdateJumpingState(true)
        humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
    end)
    if not ok then
        state.pendingJump = nil
        state.jumpFailedToken = token
        state.lastError = "Normal jump request failed: " .. safeText(errorMessage)
        state.status = "JUMP ERROR"
        addEvent("jump_error", { token = token, error = state.lastError })
        renderGui(true)
        return false
    end
    addEvent("jump_requested", {
        token = token,
        heightGap = jumpPlan.heightGap,
        travelSeconds = jumpPlan.travelSeconds
    })
    renderGui(true)
    return true
...
state.connection = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    local commitLead, commitTravelTime = effectiveLeadSeconds(
        effectiveSaveDistance,
        directionAlignment
    )
    local token = releaseToken(ball)
    local jumpPlan = getJumpPlan(
        character,
        humanoid,
        ball,
        goal,
    ...
    local saveAction = jumpPlan.recommended and "JUMP" or "DIVE"
    local actionLead = saveAction == "JUMP"
        and jumpPlan.commitLead
        or commitLead
    ...
    if state.pendingJump or state.activeJump or isAirborne(character, humanoid) then
        state.phase = "RECOVER"
        state.status = state.pendingJump and "JUMP COMMITTING"
            or state.activeJump and "JUMP RECOVERY"
            or "AIRBORNE"
        renderGui()
        return
    end
    if prediction.time > actionLead then
    ...
    if saveAction == "JUMP" then
        if state.lastJumpToken == token then
            state.status = state.autoJump and "JUMP COMMITTED" or "THREAT"
            renderGui()
            return
        end
        local firstThreat = not state.activeShot or not state.activeShot.threatRegistered
        if firstThreat then
            state.threats = state.threats + 1
            if state.activeShot then
                state.activeShot.threatRegistered = true
            end
        end
        state.status = "THREAT"
        addEvent(firstThreat and "jump_threat" or "jump_threat_retry", state.lastPrediction)
        log(string.format(
            "High central threat in %.2fs at X %.2f / Y %.2f; normal jump",
            prediction.time,
            prediction.position.X,
            prediction.position.Y
        ))
        requestJump(character, humanoid, token, prediction, jumpPlan)
        return
    end
