...
const releaseChecksum = (await readFile("tools/RbaAutoexecManager/releases/RBA.Autoexec.Manager.v1.0.2.sha256", "utf8")).trim();
const releaseManifest = JSON.parse(await readFile("tools/RbaAutoexecManager/releases/RBA.Autoexec.Manager.v1.0.2.manifest.json", "utf8"));
...
assert.match(service, /"loader-storage"/);
assert.match(service, /MoveOutsideAutoexec/);
assert.match(service, /status\.ResidualPaths/);
...
assert.equal(releaseManifest.version, "1.0.2");
